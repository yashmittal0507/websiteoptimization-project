...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // Moved the dx variable out of the loop as it is required to be computed once
    // Computed the new width of the pizzas out of the loop
    var pizzaCounter = document.querySelectorAll(".randomPizzaContainer")[0];
    var dx = determineDx(pizzaCounter, size);
    var newwidth = (pizzaCounter.offsetWidth + dx) + 'px';
...
function updatePositions() {
  ...
  var bodyCounter = (document.body.scrollTop / 1250);
  // calculate phase number for all possible outcomes of n % 5
  var phaseCounter = [
                    Math.sin(bodyCounter + 0),
                    Math.sin(bodyCounter + 1),
                    Math.sin(bodyCounter + 2),
                    Math.sin(bodyCounter + 3),
                    Math.sin(bodyCounter + 4)
                  ];

  for (var i = 0; i < items.length; i++) {
   
    var phase = phaseCounter[i%5];
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var siterows = Math.round(window.screen.height / s);
  var pizzaCount = siterows * cols;
  //Changed querySelector call to getElementById, saved this DOM call to local variable, rollingPizzas, outside of the for loop.
  var rollingPizzas = document.getElementById("movingPizzas1");
  for (var i = 0; i < pizzaCount; i++) {
    ...
    rollingPizzas.appendChild(elem);
  }
  updatePositions();
});
